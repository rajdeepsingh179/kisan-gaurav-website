...
export default function AdminPasswordPage(){
  ...
  const submit=async(event)=>{event.preventDefault();if(newPassword!==confirm){setError("New passwords do not match.");return;}setBusy(true);setError("");try{await apiFetch("/api/admin/account/password",{method:"PATCH",body:JSON.stringify({currentPassword,newPassword})});await refreshSession();navigate("/admin/login",{replace:true});}catch(reason){setError(reason.message);}finally{setBusy(false);}};
