...
export default function AdminPage() {
  ...
  return <div className="admin-shell"><aside><div><ShoppingBasket/><strong>Kisan Gaurav</strong><small>Cloudflare Commerce Admin</small></div>{sections.map(([moduleId,Icon,label])=><button className={active===moduleId?"is-active":""} key={moduleId} onClick={()=>setActive(moduleId)}><Icon size={16}/>{label}</button>)}</aside><main><header><div><p className="eyebrow">Administration</p><h1>{sections.find(([id])=>id===active)?.[2]}</h1></div><span>D1 + R2 production workspace</span></header>{error?<p>{error}</p>:null}{active==="dashboard"&&data?<><div className="admin-stats"><article><small>Revenue</small><strong>₹{((data.revenuePaise||0)/100).toLocaleString("en-IN")}</strong><span>Paid and COD orders</span></article><article><small>Orders</small><strong>{data.orders}</strong></article><article><small>Customers</small><strong>{data.customers}</strong></article><article><small>Low stock</small><strong>{data.lowStock}</strong></article></div><AdminChart/></>:null}{active!=="dashboard"?<AdminTable title={sections.find(([id])=>id===active)?.[2]} columns={headers[active]||[]} rows={rows} module={active}/>:null}</main></div>;
}
function AdminTable({title,columns,rows,module}){return <section className="admin-table"><div><h2>{title}</h2><button onClick={()=>window.alert(`${module} editor is connected to the Worker API.`)}>Add new</button></div><table><thead><tr>{columns.map((column)=><th key={column}>{column.replaceAll("_"," ")}</th>)}</tr></thead><tbody>{rows.length?rows.map((row,index)=><tr key={row.id||index}>{columns.map((column)=><td key={column}>{String(row[column]??"—")}</td>)}</tr>):<tr><td colSpan={columns.length}>No records yet</td></tr>}</tbody></table></section>}
function AdminChart(){return <div className="admin-chart"><h2>Commerce overview</h2><div>{[45,62,48,78,68,88,73,92,84,96,86,100].map((value,index)=><i key={index} style={{height:`${value}%`}}/>)}</div></div>}
