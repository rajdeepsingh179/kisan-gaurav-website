...
import { JsonMediaTextarea, MediaField } from "./MediaPicker";
...
export default function ContentWorkspace({ rows, onReload, setError, setNotice }) {
  ...
  return <div className="content-cms">
    ...
    {editor ? <ContentEditor value={editor} type={type} onClose={() => setEditor(null)} onSaved={async () => { setEditor(null); setNotice("Content saved"); if (["menus", "emails"].includes(type)) await refreshSystem(); else await onReload(); }} setError={setError} setNotice={setNotice} /> : null}
...
function EntryFields({ form, update, setError, setNotice }) {
  return <div className="content-form">
    ...
    <JsonMediaTextarea label="Structured content" value={form.content} onChange={(value) => update("content", value)} rows={18} folder={form.entryType === "blog" ? "blog" : "cms"} help="Text and layout remain structured JSON. Insert image and document URLs directly from the shared Media Library." setError={setError} setNotice={setNotice} />
    <details><summary>SEO, social cards and schema markup</summary><JsonMediaTextarea label="SEO metadata" value={form.seo} onChange={(value) => update("seo", value)} rows={10} folder="seo" help="Use Media Library assets for social images." setError={setError} setNotice={setNotice} /></details>
...
function MenuFields({ form, update, setError, setNotice }) { return <div className="admin-form-grid"><Field label="Label" value={form.label} onChange={(value) => update("label", value)} required /><Field label="URL" value={form.url} onChange={(value) => update("url", value)} required /><Field label="Location" value={form.menuLocation} onChange={(value) => update("menuLocation", value)} select={["main","footer_quick","footer_support","footer_policies"]} /><Field label="Parent item ID" value={form.parentId || ""} onChange={(value) => update("parentId", value)} /><Field label="Description" value={form.description || ""} onChange={(value) => update("description", value)} wide textarea /><MediaField label="Menu image" value={form.mediaUrl ? { id: form.mediaId, url: form.mediaUrl, file_name: "Menu image" } : ""} returnAsset folder="cms" onChange={(asset) => { update("mediaId", asset?.id || null); update("mediaUrl", asset?.url || ""); }} setError={setError} setNotice={setNotice} /><Field label="Display order" value={form.sortOrder || 0} onChange={(value) => update("sortOrder", Number(value))} type="number" /><Toggle label="Mega menu" value={form.megaMenu} onChange={(value) => update("megaMenu", value)} /><Toggle label="Enabled" value={form.enabled} onChange={(value) => update("enabled", value)} /></div>; }
