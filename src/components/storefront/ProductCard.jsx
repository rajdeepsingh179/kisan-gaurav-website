  ...
  return (
    <article className="product-card">
      <Link className="product-card__media" to={`/shop/${item.slug}`} aria-label={`View ${item.name}`}>
        ...
        <button className={`wishlist-button ${wished ? "is-active" : ""}`} type="button" onClick={(event) => { event.preventDefault(); toggleWishlist(item.slug); }} aria-label={wished ? `Remove ${item.name} from wishlist` : `Add ${item.name} to wishlist`}><Heart size={16} fill={wished ? "currentColor" : "none"} /></button>
        <img
